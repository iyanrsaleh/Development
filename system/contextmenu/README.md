...
> Status: **DIIMPLEMENTASIKAN** (contoh: extension Development,
...
## 1. Kebutuhan (dari contoh nyata `templates/distro/Development/index.js`)
...
```
templates/distro/Development/system/contextmenu/
...
```js
// templates/distro/Development/system/contextmenu/contes1.js
...
```js
// templates/distro/Development/system/contextmenu/index.js
...
1. Panggil `buildContextMenuTemplate()` langsung dengan `activeExtensionId:
   'Development'`, `contextMenuTargetId: 'contes1'` — item pertama hasil
...
## 7. Fase implementasi — STATUS: DIIMPLEMENTASIKAN (contoh Development)
...
## 7c. Target DINAMIS — tile launcher (`nxLauncherItem.js`)

**Status: DIIMPLEMENTASIKAN.** Pola sama §7a (`match` + prefix), bukan
satu file per shortcut:

- Tiap tile di `renderShortcutLauncher` /
  `system/shortcut/index.js` → `id="nxlauncher::<id-encoded>"`.
- Modul `nxLauncherItem.js`: menu **Rename** → edit judul **inline**
  di tile (bukan dialog/`prompt`) → `updateLauncherShortcut` →
  `refreshShortcutLauncher`.
- REGISTRY: `match: (id) => id.startsWith('nxlauncher::')`.
...
> `templates/distro/Development/system/titlebar/index.js` (pola konvensi
