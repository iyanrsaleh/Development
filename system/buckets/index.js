// Bucket IndexedDB TERPISAH milik distro "Development" — TIDAK PERNAH
 ...
 * @param {string} distroId — id distro (mis. "Development"), dipakai
...
export function initDistroBuckets(distroId, stores, version = 1) {
  ...
  // Reuse hanya jika dbName + version sama. Naik version (tambah store)
  // harus buka ulang supaya onupgradeneeded jalan.
  if (dbPromise && cachedDbName === dbName && cachedDbVersion === version) {
    return dbPromise;
  }
  cachedDbName = dbName;
  cachedDbVersion = version;
