...
```js
// templates/distro/Development/system/index.js
try {
  // Store "nx-launcher" = layout shortcut/launcher user (urutan, rename).
  // Detail: system/shortcut/README.md
...
```js
// Sebelum: initDistroBuckets('Development', ['nx-ssh-table-cache'], 2);
// Nambah "nx-launcher" -> WAJIB naikkan version (contoh: 3)
await initDistroBuckets('Development', ['nx-ssh-table-cache', 'nx-launcher'], 3);
```
...
- **Uninstall distro TIDAK menghapus database ini** — `POST
  ...
  Development: DevTools browser → tab Application → IndexedDB →
